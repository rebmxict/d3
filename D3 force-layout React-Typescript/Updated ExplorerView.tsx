...
class Explorer extends React.Component<any, any> {
  constructor() {
    ...
    async click (d) {
        this.mousef = 1;
        this.node.selectAll("circle").style("fill", function(o) {
            if (d.id == o.id) {
                return 'red';
            } else {
                return 'black';
        ...
        this.textpath.style("fill-opacity", 0);

        this.textpath.style("fill-opacity", function(o) {
        ...
        let data_temp = await NodeFactory.populateRelations(collection_sid);
        let temp_graph = this.graph;
        let temp_relation_data = [], temp_node_data = [];
        let tmpe_node_data_elem = {}, temp_data_elem = {};
        let tn = 1, tl = 1;
    
        data_temp["nodes"].forEach(function(d, i) {
            temp_graph.nodes.forEach(function(dp, i) {
                if(d.id == dp.id) {
                    tn = 0;
                }
            });
        ...
        data_temp["relations"].forEach(function(d, i) {
            if(d.sourceNodeId != d.targetNodeId) {
                temp_graph.links.forEach(function(dp, i) {
                    if((d.sourceNodeId == dp.target.id && d.targetNodeId == dp.source.id) ||
                        (d.sourceNodeId == dp.source.id && d.targetNodeId == dp.target.id)) {
                            tl = 0;
                        }
                });
            }
        ...
        if (tn == 1)
            this.graph.nodes.push(tmpe_node_data_elem);
        if (tl == 1)    
            this.graph.links.push(temp_data_elem);
    ...
    update() {
        ...
        this.textpath = this.svg.selectAll(".paths_s").selectAll(".pathtext")
            .data(links)

        this.textpath.exit().remove();

        var textpathEnter = this.textpath.enter()
            .append("text")
            .attr("class", "pathtext")
        ...
        this.textpath = textpathEnter.merge(this.textpath)
  ...
  async componentDidMount() {
    ...
    let data_temp = {};
    data_temp = await NodeFactory.populateRelations(collection_sid);
